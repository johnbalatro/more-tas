...
import time
import sys
def animate_text_everything(text, delay=0.1):
    for char in text:
        sys.stdout.write(colorama.Fore.LIGHTBLACK_EX + char)
        sys.stdout.flush()
        time.sleep(delay)
    print()
def animate_text_everything_empasis(text, delay=0.2):
    for char in text:
        sys.stdout.write(colorama.Fore.LIGHTBLACK_EX + char)
        sys.stdout.flush()
        time.sleep(delay)
    print()
def animate_text_shaman(text, delay=0.1):
    for char in text:
        sys.stdout.write(colorama.Fore.LIGHTGREEN_EX + char)
        sys.stdout.flush()
        time.sleep(delay)
    print()
def non_main_speak(text, delay=0.1):
    for char in text:
        sys.stdout.write(char)
        sys.stdout.flush()
        time.sleep(delay)
    print()
def minako_speak(text, delay=0.1):
    for char in text:
